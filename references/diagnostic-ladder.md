...
Publishing volume is the answer to a traffic problem, never to a trust problem. Name which one you have before anyone writes a word. Publishing more when the binding constraint is gate 6 is the expensive kind of wrong — it burns a quarter and the number does not move.
